...
// ✅ Only return a valid 2-letter country code (ISO 3166-1 alpha-2, lowercase)
const getCountry = () => {
  try {
    ...
    // Prefer explicit country code fields
    const codeFromAddr =
    ...
    // If the field is already 2 letters in `country`, accept it
    if (addr.country && /^[A-Za-z]{2}$/.test(String(addr.country))) {
      return String(addr.country).toLowerCase();
    }

    // Fallbacks: parse region from lang (en-US -> US)
...
const MetaPixel = ({pixelId}) => {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (country) am.country = country; // ✅ Pixel advanced matching country
