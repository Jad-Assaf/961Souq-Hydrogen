...
const STOP_WORDS = new Set([
  ...
  'no', 'not', 'only', 'just', 'more', 'most', 'many', 'much', 'few', 'little',
  'other', 'another', 'same', 'different', 'new', 'old', 'good', 'bad', 'best',
  'better', 'big', 'small', 'large', 'long', 'short', 'high', 'low', 'great',
  'very', 'too', 'so', 'such', 'here', 'there', 'up', 'down', 'out', 'off',
  'over', 'under', 'again', 'further', 'then', 'once', 'about', 'into', 'through',
  'during', 'before', 'after', 'above', 'below', 'between', 'among', 'within',
  'without', 'throughout', 'beside', 'besides', 'beyond', 'across', 'around',
  'near', 'far', 'inside', 'outside', 'behind', 'beyond', 'toward', 'towards',
  'upon', 'against', 'along', 'amid', 'amongst', 'amidst', 'via', 'per',
  'plus', 'minus', 'except', 'including', 'excluding', 'concerning', 'regarding',
  'considering', 'following', 'including', 'excluding', 'according', 'depending'
]);

// Common color names to filter out
const COLORS = new Set([
  'red', 'blue', 'green', 'yellow', 'orange', 'purple', 'pink', 'brown',
  'black', 'white', 'gray', 'grey', 'silver', 'gold', 'beige', 'tan',
  'navy', 'maroon', 'olive', 'lime', 'aqua', 'cyan', 'teal', 'turquoise',
  'violet', 'indigo', 'magenta', 'coral', 'salmon', 'khaki', 'ivory',
  'cream', 'peach', 'lavender', 'plum', 'mint', 'amber', 'bronze', 'copper',
  'charcoal', 'slate', 'burgundy', 'crimson', 'emerald', 'jade', 'ruby',
  'sapphire', 'topaz', 'amethyst', 'coral', 'pearl', 'ivory', 'ebony'
]);
...
function filterSuggestion(suggestion) {
  ...
  const allStopWordsOrColors = words.every(word => 
    STOP_WORDS.has(word) || COLORS.has(word)
  );
...
export async function generateSearchSuggestions(originalQuery, context) {
  ...
  try {
    ...
    const addSpelling = (term, source = 'title') => {
      ...
      if (cleaned.length >= 2 && cleaned.length < 45 && !seenSpellings.has(lower)) {
    ...
    for (const s of cleanedSuggestions) {
      ...
      for (const existing of seenNormalized) {
        ...
        const cleanExisting = existing.replace(/[^a-z0-9\s]/g, ' ').replace(/\s+/g, ' ').trim();
        const cleanCurrent = normalized.replace(/[^a-z0-9\s]/g, ' ').replace(/\s+/g, ' ').trim();
    ...
    const finalSuggestions = unique.length > 0 ? unique : filteredCorrectSpellings;
