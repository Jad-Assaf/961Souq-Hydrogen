...
import MobileFullscreenInstagramVideo from '~/components/MobileFullscreenInstagramVideo';
import MobileInstagramVideoBanner from '~/components/MobileFullscreenInstagramVideo';
...
// Add this function somewhere in your index.jsx (or a small utils file you import)

// Updated function for index.jsx
// Fixes:
// 1) Keeps .header transparent when scroll is 0
// 2) Switches the HEADER LOGO to a white variant when scroll is 0
// 3) Cart icon rule: only .cart-button svg fill -> white
// 4) Bottom nav shrink/expand from the middle

function setupTopHeaderScrollStyles({
  topThreshold = 2,
  fadeMs = 220,

  // Logo swap
  whiteLogoSrc = 'https://cdn.shopify.com/s/files/1/0552/0883/7292/files/961souqLogo-white_6a233cc8-9b7b-415c-b352-84aac4668966.png?v=1762774820', // global fallback white logo
  useDataAttrWhiteLogo = true, // prefers data-white-src on each logo image
  whiteLogoSelector = '.header-logo, .desk-nav-logo img',

  // Bottom nav animation
  bottomNavHiddenScale = 0.75,
  ...
  // Header SVGs (exclude anything inside cart button)
  const allHeaderSvgNodes = Array.from(
    document.querySelectorAll('.header svg, .header svg *'),
  );
  const headerSvgNodes = allHeaderSvgNodes.filter(
    (el) => !el.closest('.cart-button'),
  );

  // Cart icon root svg only
  const cartSvg = document.querySelector('.cart-button svg');

  // Mobile menu toggle strokes
  const mobileToggleSvgNodes = document.querySelectorAll(
    '.mobile-menu-toggle svg *',
  );

  // Search submit (support both class names)
  const searchSubmit =
    document.querySelector('.search-submit') ||
    document.querySelector('.search-bar-submit');

  const searchSubmitSvgNodes = document.querySelectorAll(
    '.search-submit svg, .search-submit svg *, .search-bar-submit svg, .search-bar-submit svg *',
  );
  ...
  // Logos
  const logoImgs = Array.from(document.querySelectorAll(whiteLogoSelector));

  // Transition strings
  const colorTransition = `color ${fadeMs}ms ease, fill ${fadeMs}ms ease, stroke ${fadeMs}ms ease, opacity ${fadeMs}ms ease`;
  const bgTransition = `background-color ${fadeMs}ms ease, opacity ${fadeMs}ms ease, background ${fadeMs}ms ease`;
  const transformTransition = `transform ${fadeMs}ms ease, opacity ${fadeMs}ms ease`;

  // Apply transitions (safe even if the feature is inactive)
  if (header) header.style.transition = bgTransition;
  if (searchForm) searchForm.style.transition = bgTransition;

  headerSvgNodes.forEach((el) => {
    if (el?.style) el.style.transition = colorTransition;
  });

  if (cartSvg?.style) cartSvg.style.transition = colorTransition;

  mobileToggleSvgNodes.forEach((el) => {
    if (el?.style) el.style.transition = colorTransition;
  });

  if (searchSubmit) searchSubmit.style.transition = bgTransition;

  searchSubmitSvgNodes.forEach((el) => {
    if (el?.style) el.style.transition = colorTransition;
  });

  if (mobileBottomNav) {
    mobileBottomNav.style.transition = transformTransition;
    mobileBottomNav.style.transformOrigin = 'center center';
    mobileBottomNav.style.willChange = 'transform, opacity';
  }

  // Store original logo src once
  ...
  function isNarrowScreen() {
    return (window.innerWidth || 0) <= maxWidth;
  }

  function getWhiteSrcForLogo(img) {
    if (!img) return null;
    if (useDataAttrWhiteLogo && img.dataset.whiteSrc) {
      return img.dataset.whiteSrc;
    }
    if (whiteLogoSrc) return whiteLogoSrc;
    return null;
  }

  function applyTopLogo() {
    logoImgs.forEach((img) => {
      const whiteSrc = getWhiteSrcForLogo(img);
      if (whiteSrc) {
  ...
  function applyTopStyles() {
    // Header + search transparent at top
    if (header) {
    ...
    // Header icons/text (excluding cart)
    headerSvgNodes.forEach((el) => {
      el.style.setProperty('color', '#fff', 'important');
      el.style.setProperty('fill', '#fff', 'important');
    });

    // Cart icon: ONLY root svg fill -> white
    if (cartSvg) {
      cartSvg.style.setProperty('fill', '#fff', 'important');
    }

    // Mobile menu toggle stroke -> white
    mobileToggleSvgNodes.forEach((el) => {
      el.style.setProperty('stroke', '#fff', 'important');
    });

    // Search submit background + icon fill
    if (searchSubmit) {
      searchSubmit.style.setProperty('background', '#fff', 'important');
      searchSubmit.style.setProperty('background-color', '#fff', 'important');
    }
    ...
    // Bottom nav: shrink from middle
    if (mobileBottomNav) {
      mobileBottomNav.style.setProperty('opacity', '0', 'important');
      mobileBottomNav.style.setProperty(
        'transform',
        `scale(${bottomNavHiddenScale})`,
        'important',
      );
      mobileBottomNav.style.setProperty('pointer-events', 'none', 'important');
  ...
  function removeTopStyles() {
    ...
    if (searchForm) {
      searchForm.style.removeProperty('background');
    ...
    if (cartSvg) {
      cartSvg.style.removeProperty('fill');
    }
    ...
    if (searchSubmit) {
      searchSubmit.style.removeProperty('background');
      searchSubmit.style.removeProperty('background-color');
    }
    ...
    if (mobileBottomNav) {
      mobileBottomNav.style.removeProperty('opacity');
      mobileBottomNav.style.removeProperty('transform');
      mobileBottomNav.style.removeProperty('pointer-events');
    }
  ...
  function update() {
    ...
    if (atTop) applyTopStyles();
    else removeTopStyles();
  }
...
export default function Homepage() {
  ...
  useEffect(() => {
    const cleanup = setupTopHeaderScrollStyles({
      topThreshold: 2,
      fadeMs: 220,

      // If you want the simplest logo swap without editing Header.jsx:
      // whiteLogoSrc: "https://cdn.shopify.com/.../your-white-logo.png",

      useDataAttrWhiteLogo: true,
      bottomNavHiddenScale: 0.75,
  ...
  return (
    <div className="home">
      ...
      <MobileInstagramVideoBanner
        // Best: direct MP4 for true video-only
        mediaUrl="https://cdn.shopify.com/videos/c/o/v/7fad0999559b4c9e892851e43aa08c60.mp4"
        // Fallback:
        headline="New arrivals are here"
        subtitle="Top picks for this week"
        buttonText="Browse collection"
        buttonHref="/collections/new-arrivals"
        overlayOpacity={0.5}
      />
