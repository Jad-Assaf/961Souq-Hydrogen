...
export const loader = async ({ request, context }: LoaderFunctionArgs) => {
  ...
  console.log('[FAQ GET] Found', productFaqs.length, 'FAQs for product', productId, 'out of', allFaqs.length, 'total FAQs');

  const faqs: { question: string; answer: string | object; name?: string; email?: string }[] = [];
  ...
  for (const node of productFaqs) {
      ...
      for (const field of node.fields) {
        if (field.key === 'faq_question') {
          ...
          console.log('[FAQ GET] Found question:', question.substring(0, 50) + '...');
        }
        if (field.key === 'faq_answer') {
          console.log('[FAQ GET] Processing answer field:', {
            ...
            looksLikeJson: field.value?.trim().startsWith('{') || field.value?.trim().startsWith('['),
          });
        ...
        if (field.key === 'faq_email') {
          email = field.value || '';
          console.log('[FAQ GET] Found email field:', email ? 'provided' : 'empty');
        }
      }
  ...
  if (faqs.length === 0 && nodesCount > 0) {
    console.error('[FAQ GET] WARNING: Found', nodesCount, 'nodes but processed 0 FAQs! This indicates a data structure issue.');
  }
